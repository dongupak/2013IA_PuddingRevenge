...
-(id) init
{
	if( (self=[super initWithColor:ccc4(255,255,255,255)] )) {
		...
		player = [CCSprite spriteWithFile:@"flame_player.png"];
        ...
        [gsound preloadEffect:@"flame_shot.mp3"];
...
- (void)ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	...
	for (int i = 1; i < 6; i++) {
		[shotanion addSpriteFrameWithFilename:[NSString stringWithFormat:@"flame_shot_%02d.png",i]];
	}
    ...
    //프로젝타일 액션
	CCSprite *projectile = [CCSprite spriteWithFile:@"flame_projectile_01.png"];
	projectile.position = ccp(winSize.width/4 + 40,winSize.height/2 + 20);
    CCAnimation *fprojanion = [[CCAnimation alloc] init];
    [fprojanion setDelayPerUnit:0.1];
	for (int i = 1; i < 5; i++) {
		[fprojanion addSpriteFrameWithFilename:[NSString stringWithFormat:@"flame_projectile_%02d.png",i]];
	}
    ...
    // 화면을 반으로 나누어 왼쪽을 터치할때와 오른쪽을 터치할 때 프로젝타일 액션이 다름
    if (location.x >= 180 && location.x < 320) {
        ...
        float velocity = 40/1 + atSpeed;  // 초당 40픽셀
        ...
        [player runAction:
         [CCSequence actions: 
          [CCRotateTo actionWithDuration:0.1 angle:cocosAngle - 180],
        ...
        //flame shot sound
        [gsound playEffect:@"flame_shot.mp3"];
    }
    ...
    if (location.x <= 180) {
        ...
        //속도 조절
        float velocity = 40/1 + atSpeed; // 480pixels/1sec
        float realMoveDuration = length/velocity;
        ...
        float angleRadians = atan2(location.x - player.position.x , location.y - player.position.y);
        float angleDegrees = CC_RADIANS_TO_DEGREES(angleRadians);
        float cocosAngle = 180 + angleDegrees;
